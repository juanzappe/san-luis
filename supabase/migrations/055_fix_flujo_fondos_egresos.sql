-- 055_fix_flujo_fondos_egresos.sql
-- Fix egreso classification in get_flujo_fondos():
--
-- 1. Retiros Socios: add DEBITO EN CUENTA, DEBITO TRANS.CAJERO AUT%,
--    N:NADAL Y ZACCAR% (was only capturing N:NADAL ANDREA / N:ZACCARO FABIAN)
-- 2. Transferencias: new CTE for BIP DB TRANSFERENCIA (was excluded but not shown)
-- 3. Gastos financieros: add VISA payments and "cuota de prestamo" pattern
-- 4. banco_deb: exclude DEBITO EN CUENTA to avoid double-counting with retiros
-- 5. New column: transferencias (position 14)
...
AS $$
  ...
  banco_clasificado AS (
    SELECT
      ...
      -- GASTOS FINANCIEROS (added VISA + cuota de prestamo)
      SUM(CASE
        WHEN concepto_lower LIKE '%haber%'
          OR concepto_lower LIKE '%sueldo%'
          OR concepto_lower LIKE '%remuner%'
