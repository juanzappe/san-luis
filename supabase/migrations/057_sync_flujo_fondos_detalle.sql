-- 057_sync_flujo_fondos_detalle.sql
-- Synchronize get_flujo_fondos_detalle() classification rules with
-- get_flujo_fondos() (migrations 055 + 056).
--
-- Changes:
--   1. retiros_detalle: add DEBITO EN CUENTA, DEBITO TRANS.CAJERO AUT,
--      N:NADAL Y ZACCAR (moved from transferencias)
--   2. transf_banco_deb: remove those 3 concepts (now only BIP DB, MP, Inviu)
--   3. banco_detalle: exclude DEBITO EN CUENTA
--   4. financieros: add VISA/MC cargos, aranceles, clearing, sellos, etc.
--      Remove federacion patr (falls to proveedores residual)
--   5. impuestos: add Santander-format patterns + concepto_norm/subcategoria
...
AS $$
  ...
  WITH banco_detalle AS (
    ...
    WHERE COALESCE(debito, 0) > 0
      ...
      -- EXCLUDE inter-account transfers (handled in transf_banco_deb)
      AND COALESCE(concepto, '') NOT LIKE 'BIP DB TRANSFERENCIA%'
      AND LOWER(COALESCE(concepto, '')) NOT LIKE '%mercado pago%'
      -- EXCLUDE Inviu (handled in transf_banco_deb)
      ...
      -- EXCLUDE partner withdrawals / own-company (handled in retiros_detalle)
  ...
  banco_clasificado AS (
    SELECT
      ...
      CASE
        ...
        THEN 'sueldos'
        -- IMPUESTOS (synced with 056 patterns)
        WHEN concepto_lower LIKE '%afip%'
          ...
          -- 057: Santander-format impuestos
        ...
        THEN 'impuestos'
        -- FINANCIEROS (synced with 056 patterns, federacion patr removed)
        WHEN concepto_lower LIKE '%comision%'
          ...
          OR concepto_lower LIKE '%com mant%'
          -- 057: VISA/MC card processing, aranceles, clearing
        ...
        THEN 'financieros'
        -- PROVEEDORES (residual — includes federacion patr)
        ELSE 'proveedores'
      END AS categoria,
      ...
      CASE
        ...
        WHEN concepto_lower LIKE 'iva 21%%'
        THEN 'IVA 21%'
        WHEN concepto_lower LIKE 'impuesto i.brutos%'
        THEN 'IMPUESTO INGRESOS BRUTOS'
        WHEN concepto_lower LIKE 'impuesto iibb%'
        THEN 'IMPUESTO IIBB'
        WHEN concepto_lower LIKE 'afip -%'
        THEN 'AFIP'
        ...
        -- 057: new financieros concepto_norm
        WHEN concepto_lower LIKE '%cargo a comercios visa%'
        THEN 'CARGO COMERCIOS VISA'
        WHEN concepto_lower LIKE '%cargo a comercios mastercard%'
        THEN 'CARGO COMERCIOS MASTERCARD'
        WHEN concepto_lower LIKE '%debito arreglo - visa%'
        THEN 'DEBITO ARREGLO VISA'
        WHEN concepto_lower LIKE '%debito arreglo - mastercard%'
        THEN 'DEBITO ARREGLO MASTERCARD'
        WHEN concepto_lower LIKE '%contracargo a comercio%'
        THEN 'CONTRACARGO COMERCIO'
        WHEN concepto_lower LIKE '%pago tarjeta de credito visa%'
        THEN 'PAGO TARJETA VISA'
        WHEN concepto_lower LIKE '%liquidacion visa%'
        THEN 'LIQUIDACION VISA'
        WHEN concepto_lower LIKE 'pago visa%'
        THEN 'PAGO VISA'
        WHEN concepto_lower LIKE '%arancel clave dni%'
        THEN 'ARANCEL CLAVE DNI'
        WHEN concepto_lower LIKE '%impuesto de sellos%'
        THEN 'IMPUESTO DE SELLOS'
        WHEN concepto_lower LIKE '%comis.gestion cheque%'
        THEN 'COMISION GESTION CHEQUE'
        WHEN concepto_lower LIKE '%com.movim mensuales clearing%'
        THEN 'COMISION CLEARING'
  ...
  -- =========================================================================
  -- RETIROS SOCIOS (synced with 055: added DEBITO EN CUENTA, CAJERO, NADAL Y ZACCAR)
  -- =========================================================================
  retiros_detalle AS (
    SELECT
      TO_CHAR(fecha, 'YYYY-MM') AS periodo,
  ...
  -- =========================================================================
  -- TRANSFERENCIAS (cleaned: only BIP DB, MP, Inviu)
  -- =========================================================================
  ...
  -- =========================================================================
  -- MP MOVEMENTS — classified (operational, unchanged)
  -- =========================================================================
  mp_clasificado AS (
    SELECT
      TO_CHAR(fecha, 'YYYY-MM') AS periodo,
